...
local function parse_chunk(chunk)
    ...
    for i, line in ipairs(chunk) do
        ...
        if func == nil then
            ...
            if
                not vim.tbl_contains({ 'Wrap comment', 'Wrap block', 'Wrap line' }, func)
            then
                error(
                    ('Failed to parse FUNCTION of test case id=%s - `%s`'):format(
                        id,
                        func
                    )
                )
        ...
        for i = cur_index, #chunk do
            ...
            if end_index == nil then
                ...
                if temp ~= nil then
                    end_index = i
                    blocks[block_index] =
                        vim.list_slice(chunk, start_index + 1, end_index - 1)
                    cur_index = i + 1
                    break
                end
                goto continue
